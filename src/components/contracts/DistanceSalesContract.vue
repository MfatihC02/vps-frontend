<template>
  <div v-if="buyerInfo && orderDetails" class="contract-container text-sm">
    ...
    <section class="mb-6">
      ...
      <div class="mb-4">
        ...
        <div class="grid grid-cols-1 gap-2">
          <p><span class="font-medium">Ünvanı:</span> {{ props.sellerInfo.companyName }}</p>
          <p><span class="font-medium">Adresi:</span> {{ props.sellerInfo.address }}</p>
          <p><span class="font-medium">Telefon:</span> {{ props.sellerInfo.phone }}</p>
          <p><span class="font-medium">E-posta:</span> {{ props.sellerInfo.email }}</p>
          <p><span class="font-medium">Mersis No:</span> {{ props.sellerInfo.mersisNo }}</p>
          <p><span class="font-medium">Vergi Dairesi/No:</span> {{ props.sellerInfo.taxInfo }}</p>
        </div>
      </div>
      ...
      <div v-if="buyerInfo" class="mb-4">
        ...
        <div class="grid grid-cols-1 gap-2">
          <p><span class="font-medium">Adı Soyadı/Ünvanı:</span> {{ buyerInfo.fullName }}</p>
          <p><span class="font-medium">Adresi:</span> {{ buyerInfo.address }}</p>
    ...
    <section class="mb-6">
      ...
      <p class="mb-4">
        2.1. İşbu sözleşmenin konusu, ALICI'nın SATICI'ya ait {{ props.sellerInfo.website }} internet sitesinden 
        elektronik ortamda siparişini verdiği aşağıda nitelikleri ve satış fiyatı belirtilen ürün/ürünlerin 
    ...
    <section class="mb-6">
      <h3 class="text-lg font-medium mb-4">4. İLGİLİ SÖZLEŞME VE POLİTİKALAR</h3>
      <p class="mb-4">4.1. Aşağıdaki konulara ilişkin detaylı bilgiler ilgili sözleşme ve politikalarda yer almaktadır:</p>
      ...
      <p class="mb-4">
        ...
        adresinde yer almakta olup, ALICI tarafından incelenmiş ve kabul edilmiştir.
      </p>
    </section>
    ...
    <section class="mb-6">
      ...
      <p class="mb-4">
        5.1. İşbu sözleşmeden doğan uyuşmazlıklarda, Gümrük ve Ticaret Bakanlığınca ilan edilen değere 
        kadar Tüketici Hakem Heyetleri, bu değerin üzerindeki uyuşmazlıklarda Tüketici Mahkemeleri yetkilidir.
      </p>
    </section>
    ...
    <section class="mb-6">
      ...
      <p class="mb-4">
        6.1. 6 (altı) maddeden ibaret bu sözleşme, taraflarca okunarak {{ formatDate(currentDate) }} 
        tarihinde ALICI tarafından elektronik ortamda onaylanmak suretiyle yürürlüğe girmiştir.
      </p>
      <p class="mb-4">
        6.2. Bu sözleşmenin bir nüshası {{ formatDate(currentDate) }} tarihinde ALICI'ya elektronik 
...
const props = defineProps({
  sellerInfo: {
    ...
    required: false,
    default: () => ({
      companyName: "Zirai Market A.Ş.",
      address: "Teknoloji Mahallesi, E-Ticaret Sokak No:123 34000 İstanbul/Türkiye",
      phone: "+90 (212) 555 0123",
      email: "info@ziraimarket.com",
      mersisNo: "0123456789000015",
      taxInfo: "İstanbul Vergi Dairesi / 123 456 7890",
      website: "www.ziraimarket.com"
    })
  }
});
...
const buyerInfo = computed(() => {
  ...
  if (!activeAddress.fullName || !activeAddress.fullAddress || 
      !activeAddress.neighborhood || !activeAddress.district || 
...
const orderDetails = computed(() => {
  ...
  console.log('Cart Items:', cartStore.items.map(item => ({
...
const subtotal = computed(() => {
  ...
  const total = orderDetails.value.items.reduce((sum, item) => sum + item.totalPriceWithoutVAT, 0);
...
const totalVAT = computed(() => {
  if (!orderDetails.value?.items) return 0;
  const total = orderDetails.value.items.reduce((sum, item) => sum + item.totalVAT, 0);
...
const uniqueProductTypes = computed(() => {
  ...
  return [...new Set(orderDetails.value.items.map(item => item.product.productType))];
});
