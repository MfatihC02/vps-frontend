  ...
  <div class="max-w-2xl mx-auto space-y-8">
    ...
    <div class="space-y-6">
      <div class="flex items-start justify-between">
        <h1 class="text-4xl font-bold text-gray-900 font-montserrat tracking-tight">{{ product.name }}</h1>
        <span class="px-4 py-2 text-sm font-medium bg-gray-50 rounded-xl text-gray-600 font-inter border border-gray-100">
          {{ product.sku }}
        </span>
      </div>
      ...
      <div class="flex items-center space-x-6">
        <div class="flex items-center">
          ...
          <span class="ml-3 text-sm font-medium text-gray-600">({{ reviewCount }} değerlendirme)</span>
        </div>
        <div class="h-6 w-px bg-gray-200"></div>
        <div class="flex items-center space-x-2">
          <Building2 class="w-5 h-5 text-[#2F5E32]" />
          <span class="text-sm font-medium text-gray-700">{{ product.brand }}</span>
        </div>
      </div>
    </div>

    <!-- Fiyat ve İndirim Bilgisi -->
    <div class="bg-gradient-to-br from-white to-gray-50 p-6 rounded-2xl shadow-sm border border-gray-100">
      <div class="flex items-baseline space-x-4">
        <span class="text-3xl font-bold text-gray-900 font-montserrat tracking-tight">
          ₺{{ formatPrice(product.price.current) }}
        </span>
        <template v-if="product.price.discount > 0">
          <span class="text-lg text-gray-400 line-through font-inter">
            ₺{{ formatPrice(calculateOriginalPrice) }}
          </span>
          <span class="inline-flex items-center px-3 py-1.5 rounded-lg text-sm font-semibold bg-red-50 text-red-600 border border-red-100">
            %{{ product.price.discount }} İndirim
          </span>
        </template>
      </div>

      <div v-if="product.price.discountEndDate" class="mt-3">
    ...
    <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 space-y-4">
      <div class="flex items-center justify-between">
        <div class="flex items-center space-x-6">
          <span class="text-gray-700 font-medium">Adet</span>
          <div class="flex items-center bg-gray-50 rounded-xl border border-gray-200">
            ...
            <span class="w-16 text-center font-medium text-gray-700">{{ quantity }}</span>
            <button
      ...
      <div class="space-y-4">
        <button
          ...
          class="w-full py-3.5 bg-gradient-to-r from-[#2F5E32] to-[#4A8E4D] text-white rounded-xl hover:from-[#2F5E32]/90 hover:to-[#4A8E4D]/90 transition-all duration-300 font-semibold text-base flex items-center justify-center space-x-3 disabled:opacity-50 disabled:cursor-not-allowed shadow-md transform hover:scale-[1.02] active:scale-[0.98]"
        >
        ...
        <a
          ...
          class="w-full py-3.5 bg-[#25D366] hover:bg-[#22BF5B] text-white rounded-xl transition-all duration-300 shadow-md transform hover:scale-[1.02] active:scale-[0.98] flex items-center justify-center space-x-3"
        >
          ...
          <span class="font-medium">Hızlı Sipariş için WhatsApp'tan Yazın</span>
        </a>
      </div>
    </div>
  </div>
