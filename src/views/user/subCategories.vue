<template>
  <div class="min-h-screen bg-gray-50/80">
    <!-- Mega Header -->
    <div
      class="bg-gradient-to-r from-primary-dark to-primary-light text-white relative overflow-hidden"
    >
      ...
      <div class="max-w-7xl mx-auto py-8 sm:py-12 px-4 relative">
        ...
        <!-- Breadcrumb -->
        <nav
          class="flex items-center gap-1.5 mt-4 text-sm text-white/90 flex-wrap font-inter"
        >
          <router-link
            to="/"
    ...
    <template v-if="hasSubCategories">
      <!-- Kontrol Paneli -->
      <div
        class="sticky top-0 bg-white/90 border-b z-10 backdrop-blur-sm shadow-sm"
      >
            ...
            <div class="flex items-center gap-3">
              <div class="relative">
                <input
                  ...
                  class="w-64 pl-10 pr-4 py-2 rounded-full border-gray-200 bg-gray-50/50 focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all font-inter text-sm"
        ...
        <div class="space-y-8">
          ...
          >
            <div
              ...
              :key="category._id"
              class="category-card bg-white rounded-xl overflow-hidden border border-gray-100/80 hover:shadow-lg hover:border-primary/10 transition-all duration-300"
            >
              <div class="flex flex-col sm:flex-row h-full">
                <div
                  class="relative w-full sm:w-1/3 aspect-[16/9] sm:aspect-auto overflow-hidden bg-gray-50"
                >
                  <img
                ...
                <div class="flex-1 p-4 flex flex-col justify-between">
                  <div>
                    <div class="flex items-start justify-between mb-2">
                      <h2
                        class="text-lg font-semibold text-gray-800 line-clamp-2 font-montserrat"
                      >
                        {{ category.name }}
                      </h2>
                    </div>
                    ...
                    <div
                      v-if="category.metadata?.keywords?.length"
                      class="sr-only"
                      aria-hidden="true"
                    >
                      <span
                        v-for="keyword in category.metadata.keywords"
                        :key="keyword"
                      >
                        {{ keyword }}
                      </span>
                    ...
                    <div class="flex items-center gap-3 text-sm text-gray-500">
                      <span
                        v-if="category.subCategoryCount > 0"
                  ...
                  <button
                    ...
                    class="mt-4 w-full bg-primary text-white py-2.5 rounded-lg hover:bg-primary-dark active:bg-primary-dark/90 transition-colors flex items-center justify-center gap-2 text-sm font-medium group font-inter"
                  >
...
const getMetaData = computed(() => {
  if (!currentCategory.value) {
    return {
      ...
      description:
        "Profesyonel üreticiler için özenle seçilmiş, yüksek kaliteli ürünler",
  ...
  return {
    ...
    description:
      category.metadata?.description ||
      category.description ||
      `${category.name} kategorisinde yer alan ürünlerimizi keşfedin`,
    keywords:
      Array.isArray(category.metadata?.keywords) &&
